...
describe('DrawioEncoder', () => {
  ...
  it('not convert for origin', () => {
    const sampleStr = `
<mxfile host="Electron">
  <diagram id="baVzHes3Wc1AAggUBBTr" name="Page-1">
    <mxGraphModel>
      <root>
        <mxCell id="0" />
      </root>
...
`;
    ...
    const data: string = DrawioEncode.decodeXml(sampleStr!)!;
    expect(data.startsWith('<mxGraphModel')).toBeDefined();
  });
